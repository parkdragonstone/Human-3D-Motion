...
def setup_backend_device(backend="auto", device="auto"):
    ...
    if device == "auto" or backend == "auto":
        ...
        # Detection and pose both run as ONNX graphs, so onnxruntime's own
        # provider list is the authority here. Torch is only used by VideoPose3D
        # lifting and says nothing about what these models can run on.
        try:
        ...
        elif "MPSExecutionProvider" in providers or "CoreMLExecutionProvider" in providers:
            device = "mps"
            backend = "onnxruntime"
