...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --storage-dir)
      ...
      ;;
    *)
      echo "Usage: bash docker/run_gpu.sh [--host-ip <windows-lan-ip>] [--storage-dir <wsl-path>]" >&2
      exit 1
      ;;
  esac
done
...
docker compose -f "$REPO_ROOT/docker/docker-compose.yml" --profile gpu up --build --force-recreate
